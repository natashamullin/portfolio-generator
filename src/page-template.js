...
const generateProjects = projectsArr => {
  ...
  ${projectsArr.filter(({ feature }) => feature)
      .map(({ name, description, languages, link }) => {
        return `
  <div class="col-12 mb-2 bg-dark text-light">${name}</h3>
  <h5 class="portfolio-languages">
  built with:
  ${languages.join(',')}
  </h5>
  <p>${description}</p>
  <a href="${link}" class="btn"><i class="fab fa-github mr-2"></i>View Project onGitHub</a>
  <?div>
  `;
      })
      .join(',')}
...
//   }
// });
// const featuredProjectHtmlArr = featuredProjectsArr.map(({ name, descrition, languages, link }) => {
//   return `
//   <div class="col-12 col-md-6 mb-2 bg-dark text-light p-3 flex-column">
//   <h3 class= "portfolio-item-title text-light">${name}</h3>
//   <h5 class= "portfolio-languages">
...
module.export = templateData => {
  ...
  //desturcture projects and about data from templateData based on their property key names
